...
from src.config import EMERGENCY_NUMBER
...
class EmergencyScenario(BaseScenario):
    ...
    def _notify_emergency_contacts(self, ctx: ScenarioContext) -> None:
        ...
        text = f"Экстренный вызов! Вызвана помощь по номеру {EMERGENCY_NUMBER}. Адрес: {HOME_ADDRESS}"
        for _id, name, phone in emergency:
            try:
                modem_sms.send_sms(ctx.modem_serial, phone, text)
                print(f"[EMERGENCY] SMS отправлено: {name} ({phone})", file=sys.stderr)
            except Exception as e:
                print(f"[EMERGENCY] Не удалось отправить SMS {name}: {e}", file=sys.stderr)

    def run(self, ctx: ScenarioContext) -> None:
        ctx.tts.say(
            f"Экстренный вызов {' '.join(EMERGENCY_NUMBER)}. "
            "Для отмены скажите нет."
        )
        ...
        # Уведомляем экстренные контакты по SMS
        self._notify_emergency_contacts(ctx)

        ctx.tts.say(f"Звоню {' '.join(EMERGENCY_NUMBER)}.")

        if ctx.modem_serial:
            from src.modem import call as modem_call
            if modem_call.dial(ctx.modem_serial, EMERGENCY_NUMBER):
                _wait_for_call_end(ctx)
            else:
                ctx.tts.say("Не удалось позвонить. Проверьте связь.")
        elif ctx.mock_modem:
            ctx.tts.say(f"Демо: экстренный вызов {' '.join(EMERGENCY_NUMBER)}.")
            _wait_for_call_end(ctx)
        else:
